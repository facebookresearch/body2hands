...
void GenerateMesh(CMeshModelInstance& returnMesh, double* resultJoint, smpl::SMPLParams& targetParam, smpl::HandModel& g_handl_model, const int regressor_type, const bool euler)
{
	...
	reconstruct_joints_mesh(g_handl_model, targetParam.handl_t.data(), targetParam.hand_coeffs.data(), targetParam.handl_pose.data(), outJ.data(), out_v.data(), dJdc, dJdP, regressor_type, euler);
	...
	returnMesh.m_alpha.reserve(pHandm->V_.rows());
	...
	for (int r = 0; r < pHandm->V_.rows(); r++)  //Vertices
	{
		...
		returnMesh.m_alpha.emplace_back(1.0);
	}
...
void GenerateMesh(CMeshModelInstance& returnMesh, double* resultJoint, smpl::SMPLParams& targetParam, TotalModel& g_total_model, const int regressor_type, const bool euler)
{
	...
	std::cout << "targetParam" << targetParam.m_adam_t << std::endl;
	// std::cout << "outV" << outV << std::endl;
	...
	returnMesh.m_alpha.resize(TotalModel::NUM_VERTICES);
	returnMesh.m_joints.clear();
	returnMesh.m_face_vertexIndices.clear();
	...
	{	
		...
		if(idx > 0 && idx < 14457) 
		{	
			// returnMesh.m_vertices[idx] = cv::Point3f(0.0, 0.0, 0.0);
			returnMesh.m_colors[idx] = cv::Point3f(0.5, 0.5, 0.45);			//Adam color
			returnMesh.m_alpha[idx] = 0.0;   // opacity
		} else if (idx < 16500) {
			returnMesh.m_colors[idx] = cv::Point3f(0.7, 0.8, 0.9);			//HAND color
			returnMesh.m_alpha[idx] = 1.0;   // opacity
		} else {
			returnMesh.m_colors[idx] = cv::Point3f(0.7, 0.8, 0.9);			//HAND color
			returnMesh.m_alpha[idx] = 1.0;   // opacity
		}
	...
	else if (regressor_type == 1)
	{
		...
		for (int i = 0; i < 21; i++)
		{
			resultJoint[3 * i] = J_coco(map_h36m_to_measure[i], 0);
			resultJoint[3 * i + 1] = J_coco(map_h36m_to_measure[i], 1);
			resultJoint[3 * i + 2] = J_coco(map_h36m_to_measure[i], 2);
		}
	...
	else
	{
		...
		resultJoint[3 * 21 + 0] = J_coco(11, 0);
		resultJoint[3 * 21 + 1] = J_coco(11, 1);
		resultJoint[3 * 21 + 2] = J_coco(11, 2);
		resultJoint[3 * 42 + 0] = J_coco(6, 0);
		resultJoint[3 * 42 + 1] = J_coco(6, 1);
		resultJoint[3 * 42 + 2] = J_coco(6, 2);
	}
}
...
// const auto start5 = std::chrono::high_resolution_clock::now();
	...
	returnMesh.m_uvs.resize(TotalModel::NUM_VERTICES);
	returnMesh.m_alpha.resize(TotalModel::NUM_VERTICES, 1.0);
...
void CopyMesh(const CMeshModelInstance& mesh, VisualizedData& g_visData)
{
	...
	g_visData.m_meshVerticesAlpha.insert(g_visData.m_meshVerticesAlpha.end(), mesh.m_alpha.begin(), mesh.m_alpha.end());
